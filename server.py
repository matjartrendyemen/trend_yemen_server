...
async def scrape_product(url):
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=Tru        context = await browser.new_context()
...
def scrape_endpoint():
    ...
    try:
        html_content = asyncio.run(scrape_product(url)from flask import Flask, request, jsonify
import asyncio
from playwright.async_api import async_playwright
